...
void UZSlideDoor::FindPressurePlate()
{
...
void UZSlideDoor::CloseDoor(float DeltaTime)
{
	//Close Door 
	CurrentZ = FMath::Lerp(CurrentZ, InitialZ, CloseSpeed *DeltaTime);
	FVector DoorLocation = GetOwner()->GetActorLocation();
	DoorLocation.Z = CurrentZ;
	GetOwner()->SetActorLocation(DoorLocation);
...
float UZSlideDoor::TotalMassOfActors()
{
